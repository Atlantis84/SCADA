...
//    qDebug()<<QString::number(this->height()*4/6);
    ...
    painter.setBrush(br);
    painter.setBrushOrigin(static_cast<int>(this->width()*8.8/10),this->height()/6);//���û�ˢ���򣬲����ã���ˢĬ��Ϊƽ�̻���
    painter.drawEllipse(eRect);
    painter.restore();

    painter.setFont(QFont(u8"KaiTi",15,QFont::Normal,false));
    painter.setPen(QColor(0,0,0));
    QRect nameRect(static_cast<int>(this->width()*8.5/10)+this->height()/2,0,static_cast<int>(this->width()*1.5/10),this->height());
    QString loginUserName = GDataFactory::get_factory()->get_current_usr_name();
