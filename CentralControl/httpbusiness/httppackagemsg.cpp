...
#include <QEventLoop>
HttpPackageMsg::HttpPackageMsg()
{

}

void HttpPackageMsg::process_http_msg(QJsonObject httpJsonObj, HttpResponse &response)
{
...
//    QNetworkAccessManager* tmpaccessManager = new QNetworkAccessManager();
    QLOG_INFO()<<"rev msg from PACKAGE!";
    try
    {
        QNetworkReply* reply;
        ...
        if((instructionValue.isString()) && (instructionValue.toString() != ""))
        {
            ...
            if(productsn.contains('/'))
            {
                ...
                ;
            //the SN is changed by shixiangkeji,process the SN
            QLOG_INFO()<<"the SN of PACKAGE request is:"<<productsn;
            QString tmpurl;
            tmpurl = "http://";
            ...
            reply = GDataFactory::get_access_manager_package()->get(request);
...
//                QLOG_INFO()<<"the response of MES is:" << QString(responseByte);
    ...
    //                                QLOG_INFO()<<"reple the VISION data is:"<<response_data;
        ...
        {
            QLOG_WARN()<<"package request msg data exist ERROR!";
            QJsonObject reJsonError;
            reJsonError.insert("code","ERROR");
            reJsonError.insert("data",u8"");
    ...
    {
        QLOG_WARN()<<"PACKAGE FLIP EXIT DETECTION";
    }
}
