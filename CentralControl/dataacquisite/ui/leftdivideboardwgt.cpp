...
#include <QGroupBox>
#include <QLabel>
LeftDivideBoardWgt* LeftDivideBoardWgt::m_pLeftDivideBoardWgt=nullptr;
LeftDivideBoardWgt::LeftDivideBoardWgt(QWidget *parent) : QWidget(parent)
{
...
void LeftDivideBoardWgt::slot_update_left_divide_ui_state(int status)
{
    if(status == 1)
...
void LeftDivideBoardWgt::slot_update_left_divide_ui_warnings(QList<QString> lstInfo)
{
    for (int i=0;i<lstInfo.length();i++)
        m_pListWgt->addItem(lstInfo[i]);
}

void LeftDivideBoardWgt::slot_update_left_divide_ui_key_part_action_times(int times)
{
